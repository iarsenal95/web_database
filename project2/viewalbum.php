<?php
	...
	//
		...
		while($row = mysql_fetch_array($result))
		{
			...
			if ($c % 5!=0)
				echo '<td><a href="./viewpicture.php?url='.$u.'&albumid='.$aid.'" target=_blank><img src= '.$u.' " width="100" height="100" /></a><br>'.$row['captain'].'<br>'.$row1['date_taken'].'</td>';
			
			else
				echo '<tr><td><a href="./viewpicture.php?url='.$u.'&albumid='.$aid.'" target=_blank><img src= '.$u.' " width="100" height="100" /></a><br>'.$row['captain'].'<br>'.$row1['date_taken'].'</td>';
			$c+=1;
		}
		echo "</table>";
